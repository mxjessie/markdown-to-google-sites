...
def eprint(*args, **kwargs):
    """ Print an error to stderr; mirrors print() """
...
#    markdown_to_google_sites(input_filename, script_output_verbose=debugging,
...
def markdown_to_google_sites(
        input_filename, print_markdown_html=False, prepend_notice_to_html=False,
        script_output_verbose=False, script_output_in_ascii=True,
    ...
    def read_file_into_html(filename):
        """ Open a file in Markdown format, return HTML """
        try:
            # load file
            ...
            return html
        except (IOError, OSError) as expn:
            print("Couldn't open file: {0}".format(expn))
            sys.exit(1)
        finally:
            input_file.close()
    ...
    for filename in glob.glob(input_filename):
        ...
        save_name = os.path.splitext(filename)[0] + '.html'
...
def main(argv=None):
    ...
    try:
        try:
            ...
            return markdown_to_google_sites(os.path.join("files", "*.md"), False)
